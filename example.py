...
if scrollYN.lower() == "y":
    ...
    while calibrated == "n":
        ...
        if calibrated == 'n':
            scrollFactor = int(input(
                'Ok. That test was with a scroll factor of %d. The lower the number, the greater the scrolling. What number would you like to try now? ' % scrollFactor))
...
def warn():
    if platform == 'win32':
        ...
        a = 440
        os.system(
            'play --no-show-progress --null --channels 1 synth %s sine %f' % (duration, a))
...
if platform == 'win32':
    ...
    while x < 2:
        os.system(
            'play --no-show-progress --null --channels 1 synth %s sine %f' % (duration, f))
        os.system(
            'play --no-show-progress --null --channels 1 synth %s sine %f' % (duration, g))
        os.system(
            'play --no-show-progress --null --channels 1 synth %s sine %f' % (duration, a))
        os.system(
            'play --no-show-progress --null --channels 1 synth %s sine %f' % (duration, g))
